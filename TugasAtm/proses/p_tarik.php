...
// Cek saldo saat ini
$sql = "SELECT saldo, pin FROM atm WHERE username='$username'";
...
if ($input_pin !== $pin) {
    echo "<script>alert('PIN salah! Silakan coba lagi.'); window.location.href='tarik.php';</script>";
    exit;
}
...
if (isset($_POST['btn_simpan'])) {
    ...
    $query = "UPDATE atm SET saldo=$saldo_baru WHERE username='$username'";
    $con->query($query);
    ...
    if ($saldo_sekarang >= $jumlah) {
        ...
        echo "<script>alert('Saldo tidak cukup!'); window.location.href='../tarik.php';</script>";
    }
}
